...
export async function fetchCurrentPrice(ticker, market) {
  try {
    ...
    if (safeMarket === 'KRX' || safeMarket === 'KOSDAQ') {
      const marketDivCode = safeMarket === 'KRX' ? '0' : '10'; // 0: 코스피, 10: 코스닥
      const domesticParams = new URLSearchParams({
        fid_cond_mrkt_div_code: marketDivCode,
        fid_input_iscd: safeTicker,
      });
      const domesticRes = await fetch(
        `${KIS_BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-price?${domesticParams}`,
        {
          method: 'GET',
          headers: buildKisHeaders({ trId: 'FHKST01010100', includeAuth: false }),
        },
      ...
      if (!domesticRes.ok) {
        return { lastPrice: null, error: { message: `KIS API 오류: ${domesticRes.status}` } };
      }
      const domesticData = await domesticRes.json();
      const price = toNumber(domesticData?.output?.stck_prpr, null);
      if (price) {
