...
export async function fetchKisBalance() {
  ...
  const balance = rows
    .map((row) => {
      ...
      const exchangeRate = toNumber(row?.bass_exrt || 0, 0);
      const buyAmountWon =
        toNumber(row?.pchs_rmnd_wcrc_amt || 0, 0) ||
        buyAmountForeign * exchangeRate;
      const evalAmountWon = evalAmountForeign * exchangeRate;
      const profitAmountWon = evalAmountWon - buyAmountWon;
      ...
      return {
        ...
        buy_amount: buyAmountWon,
        eval_amount: evalAmountWon,
        profit_amount: profitAmountWon,
  ...
  const inferredBuyAmount = balance.reduce(
    (sum, item) => sum + (item.buy_amount || 0),
    0,
  );
  const inferredEvalAmount = balance.reduce(
    (sum, item) => sum + (item.eval_amount || 0),
    0,
  );
  const inferredProfitAmount = inferredEvalAmount - inferredBuyAmount;

  const buyAmountRaw = firstDefinedValue(
    summary?.pchs_amt_smtl,
    summary?.pchs_amt_smtl_amt,
  );
  const profitAmountRaw = firstDefinedValue(
    summary?.evlu_pfls_amt_smtl,
    summary?.tot_evlu_pfls_amt,
  );
  const evalAmountRaw = firstDefinedValue(
    summary?.evlu_amt_smtl,
    summary?.evlu_amt_smtl_amt,
  );
  const totalAssetRaw = firstDefinedValue(summary?.tot_asst_amt);
  const totalDepositRaw = firstDefinedValue(
    summary?.tot_frcr_cblc_smtl,
    summary?.tot_dncl_amt,
    summary?.frcr_use_psbl_amt,
  );

  const buyAmount =
    buyAmountRaw !== undefined
      ? toNumber(buyAmountRaw, inferredBuyAmount)
      : inferredBuyAmount;
  const profitAmount =
    profitAmountRaw !== undefined
      ? toNumber(profitAmountRaw, inferredProfitAmount)
      : inferredProfitAmount;
  const evalAmount =
    evalAmountRaw !== undefined
      ? toNumber(evalAmountRaw, inferredEvalAmount)
      : buyAmount + profitAmount || inferredEvalAmount;
  const totalAsset =
    totalAssetRaw !== undefined
      ? toNumber(totalAssetRaw, evalAmount)
      : evalAmount;
  const totalDepositAmount =
    totalDepositRaw !== undefined
      ? toNumber(totalDepositRaw, 0)
      : 0;
  const depositAmount =
    totalDepositAmount > 0
      ...
      : totalAsset > 0 && evalAmount > 0
        ...
        : toNumber(
            output2?.[0]?.frcr_evlu_amt2 || output2?.[0]?.frcr_dncl_amt_2 || 0,
            0,
          );
  ...
  const summaryView = {
    ...
    withdrawableAmount: toNumber(summary?.wdrw_psbl_tot_amt || 0, 0),
    cashAvailableAmount: toNumber(summary?.frcr_use_psbl_amt || 0, 0),
  };
