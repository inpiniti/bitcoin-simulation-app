...
async function fetchKisBalanceInternal(currencyDivision) {
  ...
  const balance = rows
    .map((row) => {
      ...
      const buyAmountWon = toNumber(row?.pchs_rmnd_wcrc_amt || 0, 0) || buyAmountForeign * exchangeRate;
  ...
  // 기본적으로 output3의 값을 사용하되, 외화 조회('02')인 경우 통화별 필드를 찾아봄
  let totalAsset = toNumber(summary?.tot_asst_amt || 0, 0);
  let evalAmount = toNumber(summary?.evlu_amt_smtl || summary?.evlu_amt_smtl_amt || 0, 0);
  let depositAmount = totalAsset - evalAmount;

  if (currencyDivision === '02') {
    // 외화 조회 시, output2에서 USD 관련 필드 확인
    const usdRow = output2.find(r => r.crcy_cd === 'USD') || output2[0];
    if (usdRow) {
      // frcr_evlu_amt2: 외화 평가 금액, frcr_dncl_amt_2: 외화 예수금
      const usdEval = toNumber(usdRow.frcr_evlu_amt2 || 0, 0);
      const usdDeposit = toNumber(usdRow.frcr_dncl_amt_2 || 0, 0);
      if (usdEval > 0 || usdDeposit > 0) {
        evalAmount = usdEval;
        depositAmount = usdDeposit;
        totalAsset = usdEval + usdDeposit;
      }
    }
  }
