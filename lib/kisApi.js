...
async function fetchKisBalanceInternal(currencyDivision) {
  ...
  // 디버그: API 원본 응답을 Supabase에 저장
  const currencyLabel = currencyDivision === '01' ? '원화' : '달러';
  console.log(`[KIS] 잔고 조회 (${currencyLabel}) API 응답:`, JSON.stringify(data, null, 2).slice(0, 500));

  (async () => {
    try {
      const { supabase } = await import('./supabaseClient.js');
      const logData = currencyDivision === '01'
        ? { krw_data: data, usd_data: null }
        : { krw_data: null, usd_data: data };

      const { error } = await supabase.from('kis_debug_logs').insert({
        ...logData,
        notes: `${currencyLabel} API 원본 응답 at ${new Date().toISOString()}`,
      });

      if (error) {
        console.warn(`[KIS] ${currencyLabel} Supabase 에러:`, error.message);
      } else {
        console.log(`[KIS] ${currencyLabel} 로그 저장 ✓`);
      }
    } catch (e) {
      console.warn(`[KIS] ${currencyLabel} 로그 저장 실패:`, e?.message);
    }
  })();
  ...
  // ── output3: 전체 평가 정보 ──────────────────────────────────
  // tot_asst_amt: WCRC_FRCR_DVSN_CD에 따라 원화(01) 또는 외화(02) 금액
  // evlu_erng_rt1(수익률), evlu_pfls_amt_smtl(평가손익): 통화 구분 없음
  ...
  const evalAmount = toNumber(summary?.evlu_amt_smtl || summary?.evlu_amt_smtl_amt || 0, 0);
  const depositAmount = totalAsset - evalAmount;
  const profitAmount = toNumber(summary?.evlu_pfls_amt_smtl || summary?.tot_evlu_pfls_amt || 0, 0);
