 ...
 * KIS 원화/외화 통합 잔고 조회
 *
 * KIS API는 WCRC_FRCR_DVSN_CD(원화/외화 구분) 파라미터에 따라
 * output1의 동일 필드명(frcr_pchs_amt, frcr_evlu_amt2 등)을
 * 원화 환산값 또는 외화값으로 반환합니다. 따라서 양쪽을 모두 조회하여
 * holdings에 통화별 값을 함께 담아야 정확한 표시가 가능합니다.
 */
export async function fetchKisFullBalance() {
  const krwData = await fetchKisBalanceInternal('01');
  const usdData = await fetchKisBalanceInternal('02');

  const usdMap = new Map(usdData.balance.map((b) => [b.ticker, b]));
  const holdings = krwData.balance.map((krw) => {
    const usd = usdMap.get(krw.ticker) || {};
    return {
      ticker: krw.ticker,
      name: krw.name,
      qty: krw.qty,
      ...
      // 외화(USD)
      avg_price_usd: usd.avg_price ?? krw.avg_price,
      current_price_usd: usd.current_price ?? krw.current_price,
      buy_amount_usd: usd.buy_amount ?? 0,
      eval_amount_usd: usd.eval_amount ?? 0,
      profit_amount_usd: usd.profit_amount ?? 0,
      // 호환성 (기존 호출자가 참조)
      avg_price: krw.avg_price,
      current_price: krw.current_price,
      buy_amount: krw.buy_amount,
      eval_amount: krw.eval_amount,
      eval_amount_foreign: usd.eval_amount ?? 0,
    };
  });
...
async function fetchKisBalanceInternal(currencyDivision) {
  ...
  // ── output1: 종목별 잔고 ─────────────────────────────────────
  // WCRC_FRCR_DVSN_CD=01 응답은 모든 금액 필드가 원화 환산값,
  // 02 응답은 외화(USD)값. 필드명(frcr_*)이 같으므로 호출 측에서 통화 구분이 필요.
  ...
  // ── output3: 전체 평가 정보 / output2: 통화별 정보 ──────────
  // 주의: output3의 tot_asst_amt, evlu_amt_smtl, evlu_pfls_amt_smtl은
  //       KIS 문서상 "원화 환산 금액"으로 명시되어 있어 WCRC_FRCR_DVSN_CD=02
  //       호출에서도 원화로 반환된다. 따라서 외화 합계는 별도 필드/계산이 필요.
  const summary = data?.output3 || {};
  const output2 = data?.output2 || [];
  const usdRow = output2.find((r) => r?.crcy_cd === 'USD') || output2[0] || {};
  const profitRate = toNumber(summary?.evlu_erng_rt1 || 0, 0); // 수익률은 통화 무관
  ...
  if (currencyDivision === '01') {
    // 원화 모드: output3 원화 필드를 그대로 사용
    totalAsset = toNumber(summary?.tot_asst_amt || 0, 0);
    evalAmount = toNumber(summary?.evlu_amt_smtl || summary?.evlu_amt_smtl_amt || 0, 0);
    depositAmount = totalAsset - evalAmount;
    profitAmount = toNumber(summary?.evlu_pfls_amt_smtl || summary?.tot_evlu_pfls_amt || 0, 0);
  } else {
    // 외화 모드:
    //   평가금액 = output3.frcr_evlu_tota (외화 평가 총액)
    //   예수금   = output2[USD].frcr_dncl_amt_2 (외화 예수금)
    //   총자산   = 평가금액 + 예수금
    //   손익     = output1 각 종목 evlu_pfls_amt2 합산 (외화 손익)
    evalAmount = toNumber(summary?.frcr_evlu_tota || 0, 0);
    depositAmount = toNumber(usdRow?.frcr_dncl_amt_2 || 0, 0);
    totalAsset = evalAmount + depositAmount;
    profitAmount = balance.reduce((sum, b) => sum + (b.profit_amount || 0), 0);
  }
...
export async function fetchCurrentPrice(ticker, market) {
  try {
    ...
    const safeMarket = String(market || 'NAS').trim().toUpperCase();
...
export async function issueWebSocketKey(appkey, appsecret) {
  try {
    ...
    if (!approval_key) {
      return {
        approval_key: null,
        error: { message: data?.msg1 || '웹소켓 접속키 발급 응답 형식이 올바르지 않아요.' },
      };
    }
