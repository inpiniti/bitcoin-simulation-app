...
function parseAccount(accountNo) {
  ...
  throw new Error('계좌번호 형식이 올바르지 않아요. 예: 12345678-01 또는 1234567801');
}

function buildKisHeaders({ trId, includeAuth = true, contentType = 'application/json; charset=utf-8' } = {}) {
...
export function setKisAuth({ accountNo, appkey, appsecret, accessToken }) {
  const parsed = accountNo ? parseAccount(accountNo) : { cano: null, accountCode: null };
  kisAuth = {
...
export async function fetchKisBalance() {
  ...
  const res = await fetch(`${KIS_BASE_URL}/uapi/overseas-stock/v1/trading/inquire-present-balance?${params.toString()}`, {
    method: 'GET',
  ...
  const balance = rows.map((row) => {
    ...
    const evalAmountForeign = toNumber(row?.frcr_evlu_amt2 || 0, buyAmountForeign);
  ...
  const summaryView = {
    ...
    buyAmount: toNumber(summary?.pchs_amt_smtl || summary?.pchs_amt_smtl_amt || 0, 0),
  ...
  const deposit = toNumber(summary?.tot_dncl_amt || summary?.dncl_amt || 0, 0);
...
export async function fetchKisPrice(ticker) {
  ...
  if (/^\d{6}$/.test(safeTicker)) {
    ...
    const res = await fetch(`${KIS_BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-price?${params.toString()}`, {
      method: 'GET',
  ...
  const params = new URLSearchParams({ AUTH: '', EXCD: 'NAS', SYMB: safeTicker });
  const res = await fetch(`${KIS_BASE_URL}/uapi/overseas-price/v1/quotations/price-detail?${params.toString()}`, {
    method: 'GET',
...
export async function submitKisOrder({ ticker, quantity, side, price }) {
  ...
  const res = await fetch(`${KIS_BASE_URL}/uapi/overseas-stock/v1/trading/order`, {
    ...
    body: JSON.stringify({
      ...
      OVRS_ORD_UNPR: Number.isFinite(orderPrice) && orderPrice > 0 ? String(orderPrice) : '0',
      ORD_SVR_DVSN_CD: '0',
      ORD_DVSN: '00',
    }),
  });
