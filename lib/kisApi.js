...
async function fetchKisBalanceInternal(currencyDivision) {
  ...
  console.log(
    `[KIS] 잔고 조회 (${currencyLabel}) API 응답:`,
  ...
  (async () => {
    try {
      ...
      const logData =
        currencyDivision === '01'
          ? { krw_data: data, usd_data: {} }
          : { krw_data: {}, usd_data: data };

      const { error, data: insertedData } = await supabase.from('kis_debug_logs').insert({
        ...logData,
        notes: `${currencyLabel} API 원본 응답 at ${new Date().toISOString()}`,
      }).select();

      if (error) {
        console.warn(`[KIS] ${currencyLabel} Supabase 에러:`, error.code, error.message, error.details);
      } else {
        console.log(`[KIS] ${currencyLabel} 로그 저장 ✓`, insertedData);
      }
    } catch (e) {
      console.warn(`[KIS] ${currencyLabel} 로그 저장 실패:`, e?.message, e?.stack);
    }
  })();
