...
export async function fetchKisFullBalance() {
  ...
  // 디버그: krwData, usdData 직접 로깅 (Supabase)
  (async () => {
    try {
      ...
      await supabase.from('kis_debug_logs').insert({
        krw_data: krwData,
        usd_data: usdData,
        notes: `Raw fetchKisBalanceInternal results at ${new Date().toISOString()}`,
      });
      console.log('[KIS] Raw 데이터 Supabase 저장 완료');
    } catch (e) {
      console.warn('[KIS] Supabase 로그 실패:', e?.message);
    }
  })();
