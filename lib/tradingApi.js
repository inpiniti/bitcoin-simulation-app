 ...
 * @param {string|null} settingName - 설정 이름 필터 (null이면 전체)
 * @param {number} limit
 * @returns {{ data: Array|null, error: object|null }}
 */
export async function fetchDlTradeLogs(settingName = null, limit = 50) {
  try {
    let query = supabase
 ...
 * @returns {{ data: Array|null, error: object|null }}
 */
export async function fetchDlTradeLogsByDate(tradeDate, settingName = null) {
  try {
    let query = supabase
      .from('auto_trade_dl_logs')
      .select('*')
 ...
 * @returns {{ data: Array|null, error: object|null }}
 */
export async function fetchTopTickersLog(settingName = null, limit = 30) {
  try {
    let query = supabase
 ...
 * @returns {{ data: Array|null, error: object|null }}
 */
export async function fetchTopTickersByDate(tradeDate, settingName = null) {
  try {
    let query = supabase
      .from('top_tickers_log')
      .select('*')
