 ...
 * setting_name 필터는 DB가 아닌 호출부에서 클라이언트 사이드로 처리
 * @param {number} limit
 * @returns {{ data: Array|null, error: object|null }}
 */
export async function fetchDlTradeLogs(limit = 50) {
  try {
    const { data, error } = await supabase
...
/**
 * 특정 날짜의 딥러닝 실행 로그 조회
 * setting_name 필터는 호출부에서 클라이언트 사이드로 처리
 * @param {string} tradeDate - YYYY-MM-DD
 * @returns {{ data: Array|null, error: object|null }}
 */
export async function fetchDlTradeLogsByDate(tradeDate) {
  try {
    const { data, error } = await supabase
      .from('auto_trade_dl_logs')
      .select('*')
 ...
 * @returns {{ data: Array|null, error: object|null }}
 */
export async function fetchTopTickersLog(limit = 30) {
  try {
    const { data, error } = await supabase
 ...
 * @returns {{ data: Array|null, error: object|null }}
 */
export async function fetchTopTickersByDate(tradeDate) {
  try {
    const { data, error } = await supabase
      .from('top_tickers_log')
      .select('*')
