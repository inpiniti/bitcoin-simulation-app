...
/**
 * 특정 종목의 일주일 종가 데이터 조회
 * trade_date 기준 이전 5일 + 이후 7일 범위
 * @param {string} ticker - 종목 코드
 * @param {string} tradeDateStr - 기준 거래일 (YYYY-MM-DD)
 * @returns {Array<{ dateStr: string, close: number }>}
 */
export async function fetchTickerWeeklyCloses(ticker, tradeDateStr) {
  try {
    // 분석일 기준 앞뒤 30일씩 (총 60일 이상 범위)
    const period1 = dateToEpoch(tradeDateStr, -30);
    const period2 = dateToEpoch(tradeDateStr, 31);
    const url = `${YF_BASE}/v8/finance/chart/${encodeURIComponent(ticker)}?period1=${period1}&period2=${period2}&interval=1d`;
    ...
    for (let i = 0; i < timestamps.length; i++) {
      ...
      if (close != null) {
        data.push({
          dateStr: tsToDateStr(timestamps[i]),
          open: opens[i] ?? close,
          high: highs[i] ?? close,
          low: lows[i] ?? close,
          close,
