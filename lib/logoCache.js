...
function buildCircularXml(rawSvg, size, clipId) {
  ...
  // viewBox 추출 (없으면 width/height 속성으로 대체)
  let vx = 0, vy = 0, vw = 100, vh = 100;
  const vbMatch = rawSvg.match(/viewBox=["']([^"']+)["']/i);
  if (vbMatch) {
    [vx, vy, vw, vh] = vbMatch[1].trim().split(/[\s,]+/).map(Number);
  } else {
    const wM = rawSvg.match(/\bwidth=["']([0-9.]+)/i);
    const hM = rawSvg.match(/\bheight=["']([0-9.]+)/i);
    vw = parseFloat(wM?.[1]) || 100;
    vh = parseFloat(hM?.[1]) || 100;
  }

  // <svg>...</svg> 사이 내용 추출
  const innerMatch = rawSvg.match(/<svg[^>]*>([\s\S]*)<\/svg>\s*$/i);
  const inner = innerMatch ? innerMatch[1].trim() : '';

  const cx = size / 2;

  // object-fit: cover → max(scale) 로 원을 꽉 채운 뒤 circle clip
  const scale = Math.max(size / vw, size / vh);
  const tx = ((size - vw * scale) / 2 - vx * scale).toFixed(2);
  const ty = ((size - vh * scale) / 2 - vy * scale).toFixed(2);
  const sc = scale.toFixed(4);

  // clip-path 는 <g> 에 적용해야 react-native-svg 에서 동작함
  return (
    `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">` +
      `<defs><clipPath id="${clipId}"><circle cx="${cx}" cy="${cx}" r="${cx}"/></clipPath></defs>` +
      `<circle cx="${cx}" cy="${cx}" r="${cx}" fill="white"/>` +
      `<g clip-path="url(#${clipId})" transform="translate(${tx},${ty}) scale(${sc})">` +
        inner +
      `</g>` +
    `</svg>`
  );
